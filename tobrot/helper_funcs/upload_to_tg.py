    ...
    if os.path.isdir(local_file_name):
        ...
        if not message.photo:
            new_m_esg = await message.reply_text(
                f"Detected {len(directory_contents)} Files <a href='tg://user?id={from_user}'>📬</a>",
                quote=True
        ...
        if os.path.getsize(local_file_name) > TG_MAX_FILE_SIZE:
            ...
            i_m_s_g = await message.reply_text(
                ...
                f"<b>Detected File Size :</b> <code>{d_f_s} 😡</code>\n"
            ...
            await i_m_s_g.edit_text(
                f"<b>Detected File Size :</b> <code>{d_f_s} 😡</code>\n"
                f"<code>{ba_se_file_name}</code> <b>splitted into {number_of_files} files.</b>\n"
...
async def upload_to_gdrive(file_upload, message, messa_ge, g_id):
    ...
    del_it = await message.edit_text(f"<a href='tg://user?id={g_id}'>🔊</a> Now Uploading to 🌐 Cloud‼️")
    ...
    if os.path.isfile(file_upload):
        ...
        await asyncio.sleep(EDIT_SLEEP_TIME_OUT)
        await messa_ge.reply_text(f"📂: <code>{file_upload}</code> <a href='tg://user?id={g_id}'>Uploaded successfully in the Cloud 💨</a>\n💾 <b>Size :</b> <code>{gjay}</code>", reply_markup=button_markup)
        #await message.edit_text(f"""📂: {file_upload} Uploaded successfully in the Cloud 💨\n\n🌐 Cloud URL:  <a href="{gau_link}">FileLink</a>\n🔗 Direct URL:  <a href="{tam_link}">IndexLink</a>""")
        ...
        button_markup = pyrogram.InlineKeyboardMarkup(button)
        await asyncio.sleep(EDIT_SLEEP_TIME_OUT)
        await messa_ge.reply_text(f"📂: <a href='tg://user?id={g_id}'>Folder Uploaded successfully to</a> <code>{tt}</code> <a href='tg://user?id={g_id}'>in the Cloud 💨</a>\n💾 <b>Size :</b> <code>{gjay}</code>", reply_markup=button_markup)
        #await asyncio.sleep(EDIT_SLEEP_TIME_OUT)
        #await messa_ge.reply_text(f"""📂: Folder Uploaded successfully to {tt} in the Cloud 💨\n\n🌐 Cloud URL:  <a href="{gau_link}">FolderLink</a>\n🔗 Index URL: <a href="{tam_link}">IndexLink</a>""")
...
async def upload_single_file(message, local_file_name, caption_str, from_user, edit_media):
    ...
    if UPLOAD_AS_DOC.upper() == 'TRUE':
        ...
        if not edit_media:
            message_for_progress_display = await message.reply_text("<b>Starting Upload of :</b> <code>{}</code>".format(os.path.basename(local_file_name)))
        ...
        try:
            ...
            if not edit_media:
                message_for_progress_display = await message.reply_text(
                    "<b>Starting Upload of :</b> <code>{}</code>".format(os.path.basename(local_file_name))
                )
